...
export class SupabaseService {
  ...
  private readonly supabaseKey =
    'sb_publishable_mUGYV5uWqho95p0FrD-XRw_vpu5qxkm';
  ...
  async registrarIngreso() {
    ...
    const { data, error } = await this.supabase.from('lista_espera').insert([
      {
        cliente_id: user.id,
  ...
  async obtenerDatosUsuario(email: string) {
    const { data, error } = await this.supabase
      .from('usuarios')
      .select('nombres, apellidos, correo_electronico, perfil, foto, dni')
